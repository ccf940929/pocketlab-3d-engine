...
if uploaded_file is not None:
    ...
    if res is not None:
        ...
        with fig_col2:
            ...
            ax2.set_xlabel(f"X-coordinate ({res['unit_str']})")
            ax2.set_ylabel(f"Y-deflection ({res['unit_str']})")
        ...
        with fig_col3:
            ...
            ax3.plot(res["x_local"], res["y_local"], 'k.', markersize=2, label='Raw Data', alpha=0.3)
            ax3.plot(res["x_local"], res["best_y_poly"], 'r-', linewidth=2.5, label=f'Polynomial Fit (Deg {res["best_deg"]})')
            ax3.plot(res["x_bspl"], res["y_bspl"], 'b--', linewidth=2.5, label='B-Spline Interpolation')
            ax3.set_xlabel(f"X-coordinate ({res['unit_str']})")
            ax3.set_ylabel(f"Y-deflection ({res['unit_str']})")
